...
sys.path.append(os.path.join(ROOT_DIR, 'tf_ops/knn_cosin'))
import tf_grouping
...
def att_pooling(feature_set, name, is_training, bn_decay, weight_decay, data_format, freeze_bn):
    ...
    f_agg = tf_util.conv2d(f_agg, d, [1, 1], name + 'att_mlp', [1, 1], 'VALID', bn=True, is_training=is_training, bn_decay=bn_decay, weight_decay=weight_decay, data_format=data_format, freeze_bn=freeze_bn)
    return f_agg

def senota_module(sequence, k, mlp, scope, mlp0=None, is_training=None, bn_decay=None, weight_decay=None, data_format='NHWC', distance='l2', activation_fn=None, shrink_ratio=None, freeze_bn=False):
    """
    Args:
    ...
    # vanilla positional information
    ...
    # Augmented positional information
    relative_dis = tf.reduce_sum(coord_diff, axis=-1, keepdims=True)
    end_points['coord'] = {'coord': coord, 'coord_grouped': coord_grouped, 'coord_diff': coord_diff, 'relative_dis': relative_dis}
    pos_enc = tf.concat([relative_dis, coord_diff, coord_expand, coord_grouped], axis=-1)
    pos_enc = tf_util.conv2d(pos_enc, 16, [1, 1], 'relat_mlp', [1, 1], 'VALID', bn=True,
                               is_training=is_training, bn_decay=bn_decay, weight_decay=weight_decay, data_format=data_format, freeze_bn=freeze_bn)
    end_points['pos_enc'] = {'pos_enc': pos_enc}
    net = tf.concat([pos_enc, net_expand, net_grouped], axis=-1)
    ...
    end_points['before_att_pool'] = net
    net = att_pooling(net, 'att_pooling', is_training, bn_decay=bn_decay, weight_decay=weight_decay, data_format=data_format, freeze_bn=freeze_bn)
    end_points['after_att_pool'] = net
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:0'):
            ...
            mlp = [32, 64, 128]
            scope = 'test'
            output = senota_module(points, k, mlp, scope, is_training=tf.constant(True, dtype=tf.bool, shape=()), bn_decay=0.9)
