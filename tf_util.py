...
def pre_batch_norm_for_conv3d(inputs, is_training, bn_decay, scope, activation_fn=tf.nn.relu, freeze_bn=False):
  """ Batch normalization on 3D convolutional maps.
...
def preact_bn_for_conv3d(inputs, is_training, bn_decay, scope, freeze_bn=False, activation_fn=tf.nn.relu):
    with tf.variable_scope(scope) as sc:
        inputs = batch_norm_for_conv3d(inputs, is_training, 
                                       bn_decay=bn_decay, scope='bn', freeze_bn=freeze_bn)
        if activation_fn is not None:
